...
"
function variable_bus_voltage(pm::_PM.AbstractACRModel; nw::Int=nw_id_default, bounded::Bool=true, report::Bool=true)
...
"
  ...
  created and it is needed for the GIC.  This function creates the vm variables to add to the WR formulation
"
function variable_bus_voltage_on_off(pm::_PM.AbstractACRModel; nw::Int=nw_id_default, bounded::Bool=true, report::Bool=true)
...
function constraint_model_voltage(pm::_PM.AbstractACRModel; nw::Int=_PM.nw_id_default)
    ...
    for i in _PM.ids(pm, nw, :bus)
        ...
        JuMP.@constraint(pm.model, vm[i]^2==(vi[i]^2+vr[i]^2))
    end
end
...
function variable_gic_current(pm::_PM.AbstractACRModel; nw::Int=nw_id_default, bounded::Bool=true, report::Bool=true)
...
"
  ...
  For example, many of the GMD problem formulations need explict vm variables, which the WR formulations do not provide
"
function constraint_model_voltage_on_off(pm::_PM.AbstractACRModel; nw::Int=_PM.nw_id_default)
