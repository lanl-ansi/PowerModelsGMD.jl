...
"Calculate dc current on ungrounded gwye-gwye transformers"
...
"Calculate dc current on ungrounded gwye-gwye auto transformers"
...
"Calculate dc current on three-winding transformers"
...
"Calculate dc current for a transmission line based on node 
voltages from GMD solve"
...
"Calculate dc current for a transformer based on node 
voltages from GMD solve"
...
"Calculate dc current for a substation based on node 
voltages from GMD solve"
...
"Transformer top-oil temperature rise in degrees C above ambient at rated power"
const default_topoil_rated_temp_rise_c = 75.0 # C

"Transformer GIC hotspot cofficient in units of degrees C per amp"
const default_hotspot_coeff = 0.63

"Transformer hotspot temperature time constant in seconds"
const default_hotspot_time_const_secs = 150.0

"Transformer top-oil temperature time constant in minutes"
const default_topoil_time_const_mins = 71.0 

"Ambient temperature in units of degrees C"
const default_ambient_temp_c = 25.0


"Calculate transformer top-toil and hot-spot temperatures
from GIC and ac powerflow results"
...
"Calculate absolute hot-spot temperature from top-oil
...
"Calculate hotspot temperature rise at the current time step
based on GMD solve and hotspot temperature rise at the 
previous time step"
...
"Calculate top-oil temperature rise at the current time step
based on ac powerflow solve and hotspot temperature rise at the 
previous time step"
...
"Apply function to a dictionary value"
...
"Apply mods specified by a path to a JSON file"
...
"Return a dictonary indexing network components by their source IDs"
...
"Add results from GMD solve into the ac case data structure"
...
function update_cost_multiplier!(data::Dict{String,<:Any})
    ...
    if haskey(data, "gmd_ne_blocker")
        for (sub, m) in subs
            if haskey(data["gmd_ne_blocker"], "$sub")
