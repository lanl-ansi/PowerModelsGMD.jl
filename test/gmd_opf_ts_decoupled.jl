@testset "TEST GMD TS DECOUPLED" begin
    ...
    @testset "B4GIC case" begin
        ...
        result = _PMGMD.run_ac_gmd_opf_ts_decoupled(case_b4gic, ipopt_solver, wf_data; setting=setting, disable_thermal=true)
        ...
        @test isapprox(result[1]["ac"]["result"]["objective"], 116408.3098; atol = 1e1)
        @test isapprox(result[5]["ac"]["result"]["objective"], 116445.2873; atol = 1e1)
        @test isapprox(result[9]["ac"]["result"]["objective"], 116536.1992; atol = 1e1)
        ...
        @test isapprox(ac_solution["branch"]["2"]["pf"], -10.0092, atol=1e-1)
        @test isapprox(ac_solution["branch"]["3"]["pf"], -10.0641, atol=1e-1)
        @test isapprox(ac_solution["branch"]["3"]["qf"], -5.7281, atol=1e-1)        
        ...
        result = _PMGMD.run_ac_gmd_opf_ts_decoupled(case_b4gic, ipopt_solver, wf_data; setting=setting, disable_thermal=false)
        for period in 1:length(wf_data["time"])
            @test result[period]["ac"]["result"]["termination_status"] == _PM.LOCALLY_SOLVED
        end
